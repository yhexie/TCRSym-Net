...
REF_DIR_DEFAULT = r"D:\AxCoding\ax-tcrsym-net\data\compare"
PRED_DIR_DEFAULT = r"D:\AxCoding\ax-tcrsym-net\data\compare\result"

OUTPUT_DIR = os.getcwd()


def _parse_floats_from_line(line: str, force_z0: bool = False):
    line = line.replace(",", " ").strip()
    if not line:
        return []
    vals = [float(x) for x in line.split()]
    if force_z0 and len(vals) >= 3:
        vals[2] = 0.0
    return vals
...
def read_sym_file(path: str):
    ...
    n1 = _parse_floats_from_line(lines[1], force_z0=True)
    n2 = _parse_floats_from_line(lines[2], force_z0=True)
    n3 = _parse_floats_from_line(lines[3], force_z0=True)
...
def _point_to_plane_dist(point_pred, point_ref, normal_ref):
    """Distance from predicted point to reference plane (point-normal form)."""
    n_ref_normalized = _normalize(normal_ref)
    if n_ref_normalized is None:
        return None

    vec = [pp - pr for pp, pr in zip(point_pred, point_ref)]
    return abs(_dot(n_ref_normalized, vec))


def _best_match_angle(ref_n, pred_normals):
    best_idx, best_ang = None, None
    for i, pn in enumerate(pred_normals):
        ...
        if best_ang is None or ang < best_ang:
            best_ang = ang
            best_idx = i
    return (best_idx, best_ang) if best_idx is not None else (None, None)
...
def _std_population(values):
    # population std: sqrt(mean((x-mean)^2)); for n<=1 return 0
    n = len(values)
    if n <= 1:
        return 0.0
    mean = sum(values) / n
...
def main():
    ...
    parser.add_argument(
        "--model_name",
        default="pct",
        choices=["pct", "mlp", "pointnet"],
        help="模型名称，决定输入和输出文件后缀 (默认: pct)",
    )
    parser.add_argument(
        "--angle_thresh_deg",
        type=float,
        default=5.0,
        help="法向量匹配的角度阈值(度) (默认: 5.0)",
    )
    ...
    for ref_path in ref_paths:
        ...
        for i, ref_n in enumerate(ref_normals):
            ...
            report_lines.append(
                ...
                f"{(f'{best_ang:.6f}' if best_ang is not None else 'NaN')}\t"
                f"{(f'{dist:.6f}' if dist is not None else 'NaN')}\t{is_match}"
            )

    with open(out_report, "w", encoding="utf-8") as f:
        f.write(
            "ref_file\tpred_file\tref_normal_idx\tbest_pred_normal_idx\tbest_angle_deg\tpoint_plane_dist\tis_match\n"
        )
        ...
        if all_best_angles:
            mean = sum(all_best_angles) / len(all_best_angles)
            med = _median(all_best_angles)
            std = _std_population(all_best_angles)
            f.write(
                f"\n所有最佳角度 (度) - 平均: {mean:.4f}, 中位数: {med:.4f}, 标准差: {std:.4f}\n"
            )

        if matched_best_angles:
            mean = sum(matched_best_angles) / len(matched_best_angles)
            med = _median(matched_best_angles)
            std = _std_population(matched_best_angles)
            f.write(
                f"匹配上的角度 (度) - 平均: {mean:.4f}, 中位数: {med:.4f}, 标准差: {std:.4f}\n"
            )

        if matched_distances:
            mean = sum(matched_distances) / len(matched_distances)
            med = _median(matched_distances)
            std = _std_population(matched_distances)
            f.write(
                f"匹配上的距离 - 平均: {mean:.4f}, 中位数: {med:.4f}, 标准差: {std:.4f}\n"
            )
