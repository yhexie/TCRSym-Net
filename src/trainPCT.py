...
parser.add_argument('--nepoch', type=int, default=60, help='max number of epochs to original')
...
def main():
    ...
    my_handler = logging.FileHandler('log_PCT1.log')
    ...
    model = AcientSymNetPCT(num_points=opt.num_points)
    model = model.to(device)
    try:
        checkpoint = torch.load(str(savel_model_path) + '/checkpoints/best_model_pct.pth')
    ...
    dataloader = torch.utils.data.DataLoader(train_dataset, batch_size = 1, shuffle=True,drop_last =True)
    dataloader_test = torch.utils.data.DataLoader(test_dataset, batch_size = 1, shuffle=True,drop_last =True)
    ...
    for epoch in range(start_epoch, opt.nepoch):
        ...
        if epoch % 5 == 0:
            savepath = str(savel_model_path) + '/model_pct.pth'
            state = {
                'epoch': epoch,
